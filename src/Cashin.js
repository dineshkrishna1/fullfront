...
const Cashin = () => {
  ...
  const handlePayingAmountChange = (e) => {
    ...
    if (enteredPayingAmount === "" || /^\d+$/.test(enteredPayingAmount)) {
      // If the entered value is empty or a valid positive integer, update the state
 ...
 <Cashlist />
  ...
  <div key={index} className="text">
    ...
    <div>
      <span style={{marginLeft:"200px",marginTop:"-27px",position:"absolute"}}>Paid Amount: {v.paidAmount}</span>
    </div>
    <div>
      <span style={{marginLeft:"400px",marginTop:"-27px",position:"absolute"}}>Remaining Amount: {v.remainingAmount}</span>
    </div>
  </div>
))}
